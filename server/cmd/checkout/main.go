...
import (
	...
	"github.com/renxingdawang/rxdw-mall/server/cmd/checkout/initialize"
...
func main() {
	...
	fmt.Println("ok tg")
	CartClient := initialize.InitCart()
	OrderClient := initialize.InitOrder()
	PaymentClient := initialize.InitPayment()
	ProductClient := initialize.InitProduct()
	srv := checkout.NewServer(&CheckoutServiceImpl{
		CartManager:    CartClient,
		OrderManager:   OrderClient,
		PaymentManager: PaymentClient,
		ProductManager: ProductClient,
	},
