...
import (
	...
	"github.com/renxingdawang/rxdw-mall/server/shared/kitex_gen/cart/cartservice"
)

func InitCart() cartservice.Client {
	...
	c, err := cartservice.NewClient(
		config.GlobalServerConfig.CartSrvInfo.Name,
		client.WithResolver(r),                                     // service discovery
		client.WithLoadBalancer(loadbalance.NewWeightedBalancer()), // load balance
		client.WithMuxConnection(1),                                // multiplexing 配置连接复用
		client.WithSuite(tracing.NewClientSuite()),                 //配置分布式追踪
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: config.GlobalServerConfig.CartSrvInfo.Name}),
	)
	if err != nil {
